...
public class CancelBookingTests : BookingServiceTestsBase
{
    ...
    private static Event CreateFutureEvent(int totalSeats) =>
        new(
            "testEvent",
            "descr",
            new DateTime(2030, 05, 20, 0, 0, 0, DateTimeKind.Utc),
            new DateTime(2030, 06, 20, 0, 0, 0, DateTimeKind.Utc),
            totalSeats);

    private static User CreateUser(string login, Roles role = Roles.User) =>
        new(Guid.NewGuid(), login, "hash", role);
}
