...
`EventService` использует Redis как внешний кэш для наиболее частых операций чтения:

- `GET /events/{id}` сохраняется по ключу `event:{id}`. Карточка отдельного события часто запрашивается повторно, в том числе во время бронирования, поэтому кэш уменьшает число обращений к PostgreSQL и снижает задержку ответа.
- Список десяти популярных событий сохраняется по ключу `events:top10`. Его формирование требует сортировки данных по продажам; один общий кэшированный результат позволяет не выполнять этот запрос для каждого клиента.

Используется подход cache-aside: сначала сервис читает Redis, а при промахе получает данные из базы и записывает их в кэш. Для карточки события TTL составляет 2 минуты: данные, включая число свободных мест, остаются достаточно свежими, а частые повторные чтения не нагружают базу. Для топ-10 TTL составляет 10 минут: рейтинг меняется медленнее, а более длительное хранение заметно снижает стоимость сортировки и выборки.

### Конфигурация Redis

Параметры подключения вынесены из кода в секцию `Redis` файла [`EventService.Web/appsettings.json`](EventService/EventService.Web/appsettings.json). Это позволяет менять адрес и тайм-ауты для локального окружения, Docker и production без перекомпиляции приложения:
...
- `EndPoint` — адрес и порт Redis-сервера.
- `ConnectTimeout` — максимальное время в миллисекундах на установление подключения.
- `SyncTimeout` — максимальное время в миллисекундах на выполнение операции Redis.

### Недоступность Redis

Кэш является необязательным слоем. Ошибки операций `GET`, `SET` и `DELETE` перехватываются и записываются в лог. Если Redis недоступен, чтение считается промахом и сервис получает актуальные данные из PostgreSQL; ошибка записи в кэш не отменяет успешный ответ. При недоступности Redis сервис продолжает работать, только без ускорения от кэширования. Если инвалидация не удалась, потенциально устаревшее значение ограничено своим TTL.

### Инвалидация при записи

Для карточки события применяется стратегия **инвалидации при записи**: после успешного `UpdateEventAsync`, `DeleteEventAsync`, а также после изменения доступных мест обработчиками подтверждения и отмены бронирования вызывается `IEventCacheInvalidator`. Он удаляет ключ `event:{id}`, и следующий запрос заново загрузит актуальные данные из базы и заполнит кэш.

Инвалидация выбрана вместо обновления значения в кэше, потому что запись может менять несколько полей события и выполняться как через HTTP-операции, так и асинхронно через Kafka. Удаление одного ключа проще, не дублирует бизнес-логику формирования DTO и исключает риск сохранить в Redis неполное или устаревшее состояние. Для общего списка топ-10 согласованность является eventual: его содержимое обновляется после TTL в 10 минут.
