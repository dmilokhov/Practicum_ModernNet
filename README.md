...
Применяется стратегия **инвалидации при записи**: после успешного `UpdateEventAsync`, `DeleteEventAsync`, а также после изменения доступных мест обработчиками подтверждения и отмены бронирования вызывается `IEventCacheInvalidator`. Он удаляет ключи `event:{id}` и `events:top10`, поэтому следующий запрос заново загрузит актуальные данные из базы и заполнит кэш.

Инвалидация выбрана вместо обновления значений в кэше, потому что запись может менять несколько полей события и выполняться как через HTTP-операции, так и асинхронно через Kafka. Удаление двух связанных ключей проще, не дублирует бизнес-логику формирования DTO и исключает риск сохранить в Redis неполное или устаревшее состояние.
